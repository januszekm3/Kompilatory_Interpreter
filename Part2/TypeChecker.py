...
class TypeChecker(NodeVisitor):
    ...
    def visit_BinExpr(self, node):
        ...
        if type == 'error':
            print "Wrong expression type (tried to use operator " + op.__str__() + " on types: " + type1 + " and " + type2 + " at " + node.line.__str__()
    ...
    def visit_FunCall(self, node):
        ...
        while funsymbol is None:
            cst = cst.getParentScope()
            if cst is None:
                print "Usage of undeclared function ( " + node.name + " ) at " + node.line.__str__()
                return "error"
            else:
        ...
        if argtypes != funsymbol.argtypes:
            print "Wrong arguments for function " + funsymbol.name + " (expected: " + funsymbol.argtypes.__str__() + " actual: " + argtypes.__str__() + " )  at " + node.line.__str__()
    ...
    def visit_Init(self, node):
        ...
        if type == 'error':
            ...
            if self.carried_info["declared_type"] == 'string':
                print "Wrong type assignment (expected: " + self.carried_info["declared_type"] + " ) at " + node.line.__str__()
                return 'error'
        else:
          if self.carried_info["declared_type"] != 'string':
                print "Wrong type assignment (expected: " + self.carried_info["declared_type"] + " ) at " + node.line.__str__()
        ...
        varsymbol = self.current_symbol_table.get(node.name)
    ...
    def visit_Condition(self, node):
        ...
        if type != 'int':
            print "Wrong condition type (expected int) at " + node.line.__str__()
            return 'error'
    ...
    def visit_Assignment(self, node):
        ...
        left = left.type
        if right == 'error':
            print "Wrong type at the right side of assignment (expected: " + left + ", actual: " + right + ") at " + node.line.__str__()
            return right
    ...
    def visit_ReturnInstr(self, node):
        ...
        if right == 'error':
            print "Wrong returned type for " + self.carried_info["funsymbol"].name + " (expected: " + left + ", actual: " + right + ") at " + node.line.__str__()
            return right
